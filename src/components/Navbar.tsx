	...
	return (
		<Link href={href} className="dark:text-[#fff]">
			<h1
				className={
					isSelected
						? "dark:bg-[#c8c8dc]/10 px-[12px] py-[8px] rounded-md bg-slate-500/20 text-sm"
						: " bg-transparent hover:bg-slate-500/10 dark:hover:bg-[#c8c8dc]/5 transition-all duration-300 text-sm ease-in-out px-[12px] py-[8px] rounded-md"
				}
			>
				{title}
			</h1>
		...
		<div>
			<Link href={href} className="dark:text-[#fff] w-[50%]">
				<h1
					className={`${
						isSelected
							? "dark:bg-[#1c1f23] bg-gray-400/30"
							: " dark:bg-[#090c0f]  bg-white"
					} border-[1px] border-gray-300/50 dark:border-slate-800/50 text-center p-3 w-full transition-all duration-300 ease-in-out`}
				>
					{title}
				</h1>
			</Link>
	...
	return (
		...
		>
			{theme ? (
				...
				<FiSun className="p-2 w-9 h-9 rounded-md font-bold dark:text-white cursor-pointer" />
			)}
		</div>
	);
};
